...
cr::dai_rosnode::IMUPublisher::IMUPublisher(const std::shared_ptr<dai::DataOutputQueue> &daiMessageQueue,
                                            ...
                                            std::shared_ptr<dai::node::XLinkOut> xlinkOut, const std::string& ref_camera) :
                                            Publisher_<dai::IMUData, ros_impl::sensor_msgs::Imu>(daiMessageQueue, nh, 30, xlinkOut),
                                            calibrationHandler(calibrationHandler), referenceCamera(ref_camera)
#ifdef HAS_ROS2
                                            ,broadcaster(nh)
#endif
                                            {
    auto cname = ref_camera;
    try {
        auto extrinsics = calibrationHandler.getImuToCameraExtrinsics(dai::CameraBoardSocket::CAM_A, true);
        imu2refcam = cr::dai_tools::tx::daiExtrinsics2Tx(extrinsics, "imu_frame", cname);
        imu2refcam.header.stamp = ros_impl::now(nh);
    } catch(std::exception& e) {
        ROS_IMPL_WARN(nh, "Cant find extrinsics for IMU and %d", (int)dai::CameraBoardSocket::CAM_A);
        imu2refcam = cr::dai_tools::tx::identityTx("imu_frame", cname);
        imu2refcam.header.stamp = ros_impl::now(nh);
    }
...
void cr::dai_rosnode::IMUPublisher::operator()(std::shared_ptr<dai::IMUData> imuData) {
...

    {
        ...
        pose_msg.header.stamp = ros_impl::now(_nh);
